...
cartRouter.route('/carrito/:id/productos')
    .get( async (req, res) => {
        try {
            ...
            if(!cartProducts.products) return res.json({message: 'Carrito no encontrado'})
    ...
    .post( async (req, res) => {
        try {
            ...
            const product = await prdcts.getById(productId)
            cart.products =  [...cart.products, product]
            
            const result = await cartsApi.update(id, carrito)
